...
bool LL1::addProd(const Prod& prod) {
  if (prod.isValid) {
    ...
    if (it != G.end()) {  // 找到产生式
      it->selection.insert(prod.selection.begin(),
                           prod.selection.end());  // 追加候选式
...
set<char> LL1::first(const string& s) {
  Prod prod = Prod(s);
  if (prod.isValid) {  // 产生式
    if (FIRST[prod.noTerminal].size() != 0)
    ...
    if (VT.find(s[0]) != VT.end())  // 终结符
      ...
      if (FIRST[s[0]].size() != 0)
        ...
        vector<Prod>::iterator it =
            find(G.begin(), G.end(), s[0]);  // 求出非终结符的产生式
        if (it != G.end()) {                 // 找到产生式
        ...
        for (auto p : G)                // 求出X的follow集合
          for (auto s : p.selection) {  // 逐个候选式找X
            ...
            if ((loc = s.find(X)) != string::npos) {  // 找到非终结符X
              set<char> f =
                  first(string(s.begin() + loc + 1, s.end()));  // 求first(b)
              FOLLOW[X].insert(f.begin(), f.end());    // 加入到follow(X)中
              if (f.find('@') != f.end()) {            // 找到@
                FOLLOW[X].erase(FOLLOW[X].find('@'));  // 删除@
                set<char> fw =
                    FOLLOW[p.noTerminal];  // 把follow(A)放入follow(X)
...
void LL1::showIndataStack() {
  for (vector<char>::reverse_iterator it = indata.rbegin(); it != indata.rend();
       ++it)
    printf("%c", *it);
}
