...
class Constructable {
 ...
 public:
  Constructable() : constructed(true), value(0) { ++num_constructor_calls; }
  ...
  auto operator=(const Constructable& src) -> Constructable& {
    EXPECT_TRUE(constructed);
    value = src.value;
  ...
  auto operator=(Constructable&& src) noexcept -> Constructable& {
    EXPECT_TRUE(constructed);
    value = src.value;
    src.value = 0;
    ++num_assignment_calls;
  ...
  auto getValue() const -> int { return abs(value); }
  ...
  static auto getNumConstructorCalls() -> int { return num_constructor_calls; }

  static auto getNumMoveConstructorCalls() -> int {
    return num_move_constructor_calls;
  }

  static auto getNumCopyConstructorCalls() -> int {
    return num_copy_constructor_calls;
  }

  static auto getNumDestructorCalls() -> int { return num_destructor_calls; }

  static auto getNumAssignmentCalls() -> int { return num_assignment_calls; }

  static auto getNumMoveAssignmentCalls() -> int {
    return num_move_assignment_calls;
  }

  static auto getNumCopyAssignmentCalls() -> int {
    return num_copy_assignment_calls;
  }

  friend auto operator==(const Constructable& c0, const Constructable& c1)
      -> bool {
...
struct NonCopyable {
  NonCopyable() = default;
  NonCopyable(NonCopyable&& /*unused*/) noexcept {}
  auto operator=(NonCopyable&& /*unused*/) noexcept -> NonCopyable& {
    return *this;
  }
...
TYPED_TEST(SmallVectorTest, PushPopTest) {
  ...
  bool requires_growth = this->theVector.capacity() < 3;
...
TYPED_TEST(SmallVectorTest, ResizeWithElementsTest) {
  ...
  size_t move_ctors = Constructable::getNumMoveConstructorCalls();
  EXPECT_TRUE(move_ctors == 0 || move_ctors == 2);
...
TYPED_TEST(SmallVectorTest, AppendRepeatedNonForwardIterator) {
  ...
  this->theVector.push_back(Constructable(1));
  this->theVector.append(OutputIterator(), OutputIterator());
  this->assertValuesInOrder(this->theVector, 3U, 1, 7, 7);
}
...
TYPED_TEST(SmallVectorTest, AppendSmallVector) {
  ...
  SmallVector<Constructable, 3> other_vector = {7, 7};
  this->theVector.push_back(Constructable(1));
  this->theVector.append(other_vector);
  this->assertValuesInOrder(this->theVector, 3U, 1, 7, 7);
}
...
TYPED_TEST(SmallVectorTest, AssignSmallVector) {
  ...
  SmallVector<Constructable, 3> other_vector = {7, 7};
  this->theVector.push_back(Constructable(1));
  this->theVector.assign(other_vector);
  this->assertValuesInOrder(this->theVector, 2U, 7, 7);
}
...
TYPED_TEST(SmallVectorTest, EraseTest) {
  ...
  this->makeSequence(this->theVector, 1, 3);
  const auto& the_const_vector = this->theVector;
  this->theVector.erase(the_const_vector.begin());
  this->assertValuesInOrder(this->theVector, 2U, 2, 3);
}
...
TYPED_TEST(SmallVectorTest, EraseRangeTest) {
  ...
  this->makeSequence(this->theVector, 1, 3);
  const auto& the_const_vector = this->theVector;
  this->theVector.erase(the_const_vector.begin(), the_const_vector.begin() + 2);
...
TYPED_TEST(SmallVectorTest, InsertEmptyRangeTest) {
  ...
  EXPECT_EQ(
      this->theVector.end(),
      this->theVector.insert(this->theVector.end(), this->theVector.begin(),
                             this->theVector.begin()));
  EXPECT_EQ(
      this->theVector.begin() + 1,
      this->theVector.insert(this->theVector.begin() + 1,
                             this->theVector.begin(), this->theVector.begin()));
}
