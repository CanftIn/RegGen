...
using NumSet = FlatSet<int64_t>;

auto Has(const NumSet& set, int64_t k) -> bool {
...
using NumSetContents = std::vector<int64_t>;
auto Contents(const NumSet& set) -> NumSetContents {
...
auto Fill(NumSet* set, int64_t start, int64_t limit) -> void {
...
using UniqInt = std::unique_ptr<int>;
...
using UniqSet = FlatSet<UniqInt, HashUniq, EqUniq>;
